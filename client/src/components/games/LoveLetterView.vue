          ...
          <div class="table-stage">
            ...
            <div class="table-center">
              ...
              <div class="live-reveal" aria-live="polite">
                ...
                <template v-else>
                  <div v-if="canDrawCard" class="reveal-placeholder draw-instruction">
                    <strong>먼저 카드 한 장을 뽑으세요</strong>
                    <span>덱을 클릭하면 카드를 받을 수 있습니다.</span>
                  </div>
                  <div v-else class="reveal-placeholder">{{ state?.actionPhase === 'draw' ? `${playerName(state?.currentTurnId)}님이 카드를 뽑기를 기다리는 중` : '이번 라운드의 첫 행동을 기다리는 중' }}</div>
                </template>
              </div>
            </div>
...
const turnStatus = computed(() => {
  if (state.value?.gamePhase === 'waiting') return { label:'게임 준비',title:'게임 시작을 기다리고 있습니다.',description:`현재 ${connectedPlayerCount.value}명입니다. 방장이 시작하면 각자 비밀 손패를 받습니다.` };
  if (state.value?.gamePhase === 'round_result') return { label:'라운드 종료',title:`${roundWinnerNames.value} 라운드 승리`,description:roundRewardCopy.value };
  if (state.value?.gamePhase === 'finished') return { label:'게임 종료',title:'편지 전달이 끝났습니다.',description:'최종 호감 토큰과 순위를 확인하고 대기실로 돌아갈 수 있습니다.' };
  if (myPlayer.value?.eliminated) return { label:'관전 중',title:'이번 라운드에서 탈락했습니다.',description:'다음 라운드까지 남은 플레이어의 공개 행동을 지켜보세요.' };
  const currentName = playerName(state.value?.currentTurnId);
  if (!isMyTurn.value) {
    const drawing = state.value?.actionPhase === 'draw';
    return {
      label:`${currentName}님의 차례 · ${drawing ? '카드 뽑기' : '카드 사용'} 단계`,
      title:drawing ? `${currentName}님이 카드를 뽑고 있습니다.` : `${currentName}님이 사용할 카드를 선택하고 있습니다.`,
      description:'공개되는 카드와 적용되는 효과를 잘 보고 기억하세요.',
    };
  }
  if (state.value?.actionPhase === 'draw') return { label:'내 차례 · 카드 뽑기 단계',title:'카드 한 장을 뽑으세요.',description:'카드를 뽑으면 손에 두 장의 카드가 생깁니다. 그중 한 장을 사용하게 됩니다.' };
  if (state.value?.actionPhase === 'chancellor') return { label:'내 차례 · 재상 선택 단계',title:'손에 남길 카드 한 장을 선택하세요.',description:'나머지 카드는 선택한 순서대로 덱 맨 아래에 돌아갑니다.' };
  return { label:'내 차례 · 카드 사용 단계',title:'사용할 카드를 선택하세요.',description:'선택한 카드는 공개되고 즉시 효과가 적용됩니다.' };
});
