<template>
  <div class="love-game">
    ...
    <section class="status-strip" aria-label="러브레터 게임 상태">
      <div><span>라운드</span><strong>{{ state?.roundCount || 0 }}</strong></div>
      <div><span>내 호감</span><strong>{{ myPlayer?.favorTokens || 0 }} / {{ state?.favorTarget || '-' }}</strong></div>
      <div><span>남은 덱</span><strong>{{ state?.deckCount || 0 }}장</strong></div>
      <div><span>생존</span><strong>{{ state?.activeCount || connectedPlayerCount }}명</strong></div>
    </section>
        ...
        <article class="table-panel">
          ...
          <div class="table-stage">
            ...
            <div class="table-center">
              <div class="deck-area">
                <LoveLetterCard :card="null" compact />
                <strong>{{ state?.deckCount || 0 }}</strong><span>남은 편지</span>
              </div>
            ...
            <LoveLetterPlayerSeat v-if="myPlayer" :player="myPlayer" :current="isMyTurn" own />
          </div>
        </article>
...
const turnStatus = computed(() => {
  ...
  return { label:isMyTurn.value?'내 차례':'현재 차례',value:`${playerName(state.value?.currentTurnId)} · ${state.value?.actionPhase === 'chancellor'?'재상 선택':'카드 사용'}` };
});
const actionStep = computed(() => state.value?.gamePhase === 'round_result'?'라운드 결과':isMyTurn.value?'내 차례':'상대 차례');
