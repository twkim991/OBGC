    ...
    <template v-else>
      <div class="play-grid">
        <article class="market-panel">
          ...
          <section class="nobles" aria-label="귀족 타일">
            <ActionGuard v-for="noble in state?.nobles || []" :key="noble.id" :reason="nobleBlockedReason(noble)" label="귀족 선택"><SplendorNoble :noble="noble" :selectable="canChooseNoble&&state.eligibleNobleIds.includes(noble.id)" :selected="selectedNobleId===noble.id" @select="selectedNobleId=$event.id" /></ActionGuard>
          </section>
          <section class="market" aria-label="개발 카드 시장">
            <div v-for="tier in [3,2,1]" :key="tier" class="tier-row">
              <ActionGuard :reason="deckBlockedReason(tier)" :label="`${tier}단계 덱 예약`" block><button type="button" class="tier-deck" :class="{selected:selectedCard?.source==='deck'&&selectedCard?.tier===tier}" :disabled="Boolean(deckBlockedReason(tier))" @click="selectDeck(tier)"><strong>{{ tier }}</strong><span>단계</span><small>{{ state.deckCounts[tier] }}장</small></button></ActionGuard>
              <div class="card-row">
                <ActionGuard v-for="card in state.markets[tier]" :key="card.id" :reason="cardSelectionBlockedReason" :label="`${tier}단계 개발 카드`"><SplendorCard :card="card" interactive :selected="selectedCard?.card?.id===card.id" :affordable="canAfford(card)" @select="selectMarketCard" /></ActionGuard>
              </div>
            </div>
          </section>
        </article>
        ...
        <aside class="side-stack">
          <section class="side-panel action-panel" aria-labelledby="splendor-action-title">
            ...
            <template v-if="canReturnTokens">
              ...
              <div class="return-grid">
                <ActionGuard v-for="color in tokenColors" :key="color" :reason="returnColorBlockedReason(color)" :label="`${colorName(color)} 토큰 반환`" block><button type="button" :disabled="Boolean(returnColorBlockedReason(color))" @click="toggleReturn(color)"><SplendorGem :color="color" /><span>{{ colorName(color) }}<b>{{ returnSelection[color]||0 }} / {{ myPlayer?.tokens[color]||0 }}</b></span></button></ActionGuard>
              </div>
              <ActionGuard :reason="returnConfirmBlockedReason" label="선택한 토큰 반환" block><button class="btn primary wide" type="button" :disabled="Boolean(returnConfirmBlockedReason)" @click="returnTokens">선택한 토큰 반환</button></ActionGuard>
            </template>
            ...
            <template v-else-if="canChooseNoble">
              ...
              <ActionGuard :reason="nobleConfirmBlockedReason" label="이 귀족 방문받기" block><button class="btn primary wide" type="button" :disabled="Boolean(nobleConfirmBlockedReason)" @click="chooseNoble">이 귀족 방문받기</button></ActionGuard>
            </template>
            ...
            <template v-else>
              ...
              <div v-if="actionMode==='gems'" class="action-buttons"><ActionGuard :reason="takeGemsBlockedReason" label="보석 가져오기"><button class="btn primary" type="button" :disabled="Boolean(takeGemsBlockedReason)" @click="takeGems">보석 가져오기</button></ActionGuard><button class="btn" type="button" @click="selectedGems={}">선택 초기화</button></div>
              <div v-else class="action-buttons"><ActionGuard :reason="purchaseBlockedReason" label="카드 구매"><button class="btn primary" type="button" :disabled="Boolean(purchaseBlockedReason)" @click="purchaseCard">카드 구매</button></ActionGuard><ActionGuard :reason="reserveBlockedReason" label="카드 예약"><button class="btn" type="button" :disabled="Boolean(reserveBlockedReason)" @click="reserveCard">카드 예약</button></ActionGuard></div>
            </template>
          </section>
          ...
          <section class="side-panel bank-panel" aria-labelledby="splendor-bank-title">
            ...
            <div class="gem-bank">
              <ActionGuard v-for="color in tokenColors" :key="color" :reason="gemColorBlockedReason(color)" :label="`${colorName(color)} 선택`" block><button type="button" :disabled="Boolean(gemColorBlockedReason(color))" :aria-pressed="Boolean(selectedGems[color])" @click="selectGem(color)"><SplendorGem :color="color" /><span><strong>{{ state.bank[color] }}</strong><small>{{ colorName(color) }}<template v-if="selectedGems[color]"> · 선택 {{ selectedGems[color] }}</template></small></span></button></ActionGuard>
            </div>
          </section>
      ...
      <section class="engine-panel">
        ...
        <div class="engine-grid">
          ...
          <div class="engine-section"><h3>예약 카드</h3><p>예약 카드의 정체는 나에게만 보이며 최대 3장입니다.</p><div class="reserved-row"><ActionGuard v-for="card in privateReservations.cards" :key="card.id" :reason="cardSelectionBlockedReason" label="예약 개발 카드"><SplendorCard :card="card" interactive :selected="selectedCard?.source==='reserved'&&selectedCard?.card?.id===card.id" :affordable="canAfford(card)" @select="selectReservedCard" /></ActionGuard><span v-if="!privateReservations.cards.length" class="empty">예약한 카드가 없습니다.</span></div></div>
        </div>
      </section>
    </template>
...
const currentPlayer=computed(()=>players.value.find(p=>p.sessionId===state.value?.currentTurnId));
const startBlockedReason=computed(()=>connectedPlayerCount.value<2?'플레이어가 2명 이상 모여야 시작할 수 있습니다.':'');
const mainActionBlockedReason=computed(()=>{if(actionPending.value)return'이전 행동을 처리하고 있습니다. 잠시 기다려주세요.';if(!isMyTurn.value)return`${currentPlayer.value?.nickname||'다른 플레이어'}님의 차례입니다.`;if(state.value?.actionPhase==='return_tokens')return'먼저 초과한 토큰을 반환하세요.';if(state.value?.actionPhase==='choose_noble')return'먼저 방문받을 귀족을 선택하세요.';return canMainAction.value?'':'지금은 주요 행동을 선택하는 단계가 아닙니다.'});
const cardSelectionBlockedReason=computed(()=>mainActionBlockedReason.value||(actionMode.value!=='cards'?'먼저 카드 행동 탭을 선택하세요.':''));
const takeGemsBlockedReason=computed(()=>{if(mainActionBlockedReason.value)return mainActionBlockedReason.value;if(actionMode.value!=='gems')return'먼저 보석 가져오기 탭을 선택하세요.';if(gemSelectionValid.value)return'';const total=selectedGemEntries.value.reduce((sum,entry)=>sum+entry.count,0);if(!total)return'서로 다른 색 보석 3개 또는 같은 색 보석 2개를 선택하세요.';return`현재 ${total}개를 선택했습니다. 서로 다른 색 3개 또는 재고가 4개 이상인 같은 색 2개를 맞춰주세요.`});
const purchaseBlockedReason=computed(()=>{if(cardSelectionBlockedReason.value)return cardSelectionBlockedReason.value;if(!selectedCard.value)return'구매할 개발 카드를 선택하세요.';if(!selectedCard.value.card)return'덱 맨 위의 비공개 카드는 바로 구매할 수 없습니다. 예약만 가능합니다.';if(!canAfford(selectedCard.value.card))return purchaseShortage(selectedCard.value.card);return''});
const reserveBlockedReason=computed(()=>{if(cardSelectionBlockedReason.value)return cardSelectionBlockedReason.value;if(!selectedCard.value)return'예약할 시장 카드나 단계 덱을 선택하세요.';if(selectedCard.value.source==='reserved')return'이미 예약한 카드는 다시 예약할 수 없습니다.';if((myPlayer.value?.reservedCount||0)>=3)return'예약 공간 3칸을 모두 사용했습니다.';return''});
const returnConfirmBlockedReason=computed(()=>{if(actionPending.value)return'토큰 반환을 처리하고 있습니다.';const remaining=returnRequired.value-returnSelectedTotal.value;return remaining===0?'':`반환할 토큰을 ${remaining}개 더 선택하세요.`});
const nobleConfirmBlockedReason=computed(()=>actionPending.value?'귀족 선택을 처리하고 있습니다.':selectedNobleId.value?'':'시장 위에서 방문받을 귀족 한 명을 선택하세요.');

function deckBlockedReason(tier){return cardSelectionBlockedReason.value||(!state.value?.deckCounts?.[tier]?`${tier}단계 덱에 남은 카드가 없습니다.`:'')}
function gemColorBlockedReason(color){if(mainActionBlockedReason.value)return mainActionBlockedReason.value;if(actionMode.value!=='gems')return'먼저 보석 가져오기 탭을 선택하세요.';if(selectedGems.value[color])return'';if(color==='gold')return'황금 토큰은 직접 가져갈 수 없으며 카드를 예약할 때만 받을 수 있습니다.';if(!state.value?.bank?.[color])return`은행에 남은 ${colorName(color)} 토큰이 없습니다.`;const total=selectedGemEntries.value.reduce((sum,entry)=>sum+entry.count,0);if(total>=3)return'이미 보석 3개를 선택했습니다. 선택을 해제한 뒤 다시 골라주세요.';if(selectedGemEntries.value.some(entry=>entry.count===2))return'같은 색 보석 2개와 다른 색 보석을 함께 가져올 수 없습니다.';return''}
function returnColorBlockedReason(color){if(actionPending.value)return'토큰 반환을 처리하고 있습니다.';if(!(myPlayer.value?.tokens?.[color]||0))return`반환할 ${colorName(color)} 토큰을 가지고 있지 않습니다.`;if(!(returnSelection.value[color]||0)&&returnSelectedTotal.value>=returnRequired.value)return'반환할 토큰 수를 모두 선택했습니다. 다른 색을 고르려면 기존 선택을 먼저 해제하세요.';return''}
function nobleBlockedReason(noble){if(!canChooseNoble.value||state.value?.eligibleNobleIds?.includes(noble.id))return'';const missing=colors.map(color=>({color,count:Math.max(0,(noble.requirement?.[color]||0)-(myPlayer.value?.bonuses?.[color]||0))})).filter(item=>item.count>0);return missing.length?`이 귀족을 방문받으려면 ${missing.map(item=>`${colorName(item.color)} 할인 ${item.count}개`).join(', ')}가 더 필요합니다.`:'이 귀족의 방문 조건을 아직 충족하지 못했습니다.'}
function purchaseShortage(card){let gold=myPlayer.value?.tokens?.gold||0;const missing=[];for(const color of colors){let need=Math.max(0,(card.cost?.[color]||0)-(myPlayer.value?.bonuses?.[color]||0)-(myPlayer.value?.tokens?.[color]||0));const covered=Math.min(gold,need);gold-=covered;need-=covered;if(need>0)missing.push(`${colorName(color)} ${need}개`)}return missing.length?`${missing.join(', ')}가 부족합니다. 할인과 황금 토큰을 포함해 계산했습니다.`:'현재 자원으로 구매할 수 없습니다.'}
const turnStatus=computed(()=>{
  if(state.value?.gamePhase==='waiting')return{label:'게임 준비',title:'게임 시작을 기다리고 있습니다.',description:`현재 ${connectedPlayerCount.value}명입니다. 방장이 시작하면 보석과 개발 카드 시장이 준비됩니다.`};
  if(state.value?.gamePhase==='finished')return{label:'게임 종료',title:state.value.lastAction||'상단 경쟁이 끝났습니다.',description:'최종 명성과 순위를 확인하고 대기실로 돌아갈 수 있습니다.'};
  const roundPrefix=state.value?.finalRoundTriggered?'마지막 라운드 · ':'';
  const playerName=currentPlayer.value?.nickname||'플레이어';
  if(!isMyTurn.value)return{label:`${roundPrefix}${playerName}님의 차례`,title:'이번 턴의 행동을 선택하고 있습니다.',description:state.value?.finalRoundTriggered?'시장과 보석 은행의 변화를 확인하세요. 현재 라운드가 끝나면 게임이 종료됩니다.':'시장과 보석 은행의 변화를 확인하세요.'};
  if(state.value?.actionPhase==='return_tokens')return{label:`${roundPrefix}내 차례 · 초과 토큰 반환 단계`,title:'토큰을 10개만 남기세요.',description:`보유 한도를 초과한 ${returnRequired.value}개를 은행에 돌려놓으면 턴이 끝납니다.`};
  if(state.value?.actionPhase==='choose_noble')return{label:`${roundPrefix}내 차례 · 귀족 선택 단계`,title:'방문받을 귀족 한 명을 선택하세요.',description:'조건을 동시에 만족한 귀족 중 한 명을 선택하면 명성 3점을 얻습니다.'};
  if(actionMode.value==='gems')return{label:`${roundPrefix}내 차례 · 보석 선택 단계`,title:'가져갈 보석을 선택하세요.',description:'서로 다른 색 3개 또는 재고가 4개 이상인 같은 색 2개를 가져올 수 있습니다.'};
  return{label:`${roundPrefix}내 차례 · 카드 행동 단계`,title:'구매하거나 예약할 카드를 선택하세요.',description:'비용을 낼 수 있으면 구매하고, 부족하다면 최대 3장까지 예약할 수 있습니다.'};
});
...
function toggleReturn(color){const current=returnSelection.value[color]||0;const owned=myPlayer.value?.tokens[color]||0;if(!owned)return;if(current>=owned||current>0&&returnSelectedTotal.value>=returnRequired.value){const next={...returnSelection.value};delete next[color];returnSelection.value=next;return}if(returnSelectedTotal.value<returnRequired.value)returnSelection.value={...returnSelection.value,[color]:current+1}}
